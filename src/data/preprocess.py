...
def process_date_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    date_columns = [
        ...
        'reform_date', 'reform_wet_area_date',
        'reform_interior_date', 'renovation_date',
        'snapshot_create_date', 'new_date',
        'snapshot_modify_date', 'timelimit_date', 'usable_date'
    ]
    ...
    for col in date_columns:
        ...
        df_processed[f'{col}_ym'] = df_processed[col].dt.year * 100 + df_processed[col].dt.month
...
def process_address_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'full_address' in df.columns:
        ...
        df_processed['city'] = df_processed['full_address'].apply(
            lambda x: x.split('市')[0] + '市' if '市' in str(x) else 
                     (x.split('区')[0] + '区' if '区' in str(x) else 
                     (x.split('町')[0] + '町' if '町' in str(x) else 
...
def preprocess_for_catboost(
    ...
    drop_cols = [
        target_col,
        'building_id', 'unit_id', 'bukken_id',  # ID系
    ]
    
    # テキスト系の列（完全に削除せず、一部は処理）
    text_cols_to_drop = [
        'building_name', 'building_name_ruby',
        'homes_building_name', 'homes_building_name_ruby',
        'unit_name', 'name_ruby',
        'empty_contents', 'parking_memo',
        'reform_place_other', 'reform_wet_area_other',
        'reform_interior_other', 'reform_exterior_other',
        'reform_etc', 'renovation_etc',
    ...
    for col in train_processed.columns:
        ...
        elif train_processed[col].dtype in ['int64', 'int32'] and train_processed[col].nunique() < 50:
