...
def create_cluster_aggregation_features(
    ...
    if target_col in train.columns:
        cluster_target_stats = train_copy.groupby(cluster_col)[target_col].agg([
    ...
    # クラスターごとの物件数
    cluster_counts = train_copy.groupby(cluster_col).size().reset_index(name='cluster_count')
    ...
    # その他の数値特徴量の集約
    for col in agg_cols:
        cluster_agg = train_copy.groupby(cluster_col)[col].agg([
...
def create_target_encoding_features(
    ...
    for col in categorical_cols:
        # カテゴリごとの統計量
        cat_stats = train_copy.groupby(col)[target_col].agg(['mean', 'count']).reset_index()
        cat_stats.columns = [col, f'{col}_target_mean', f'{col}_count']
        
        # スムージング
        cat_stats[f'{col}_target_encoded'] = (
            (cat_stats[f'{col}_target_mean'] * cat_stats[f'{col}_count'] + global_mean * smoothing) /
...
def create_distance_features(
    ...
    for city_name, (city_lat, city_lon) in major_cities.items():
        df_copy[f'distance_to_{city_name}'] = np.sqrt(
            (df_copy[lat_col] - city_lat) ** 2 + 
            (df_copy[lon_col] - city_lon) ** 2
        )
...
def create_derived_features(
    ...
    if 'money_kyoueki' in df.columns and 'money_room' in df.columns:
        df_copy['kyoueki_ratio'] = df_copy['money_kyoueki'] / (df_copy['money_room'] + 1)
        feature_count += 1
